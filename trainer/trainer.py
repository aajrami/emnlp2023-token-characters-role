...
"""
The Trainer class, to easily train Transformers from scratch or finetune it on a new task.
"""
...
class HybroEmbedding(Module):
    ...
    def __init__(self, ori_embedding: nn.Embedding, freeze_idx_upper: int=3, device=None):
        super(HybroEmbedding, self).__init__()
        self.num_embeddings = ori_embedding.num_embeddings
        self.embedding_dim = ori_embedding.embedding_dim
        self.padding_idx = ori_embedding.padding_idx
        self.max_norm = ori_embedding.max_norm
        self.norm_type = ori_embedding.norm_type
        self.scale_grad_by_freq = ori_embedding.scale_grad_by_freq
        self.sparse = ori_embedding.sparse

        freeze_special_token_embs = nn.Embedding(freeze_idx_upper, self.embedding_dim, padding_idx=1)
        freeze_special_token_embs.weight.data.copy_(
            ori_embedding.weight.data[0:freeze_idx_upper]
        )
        freeze_special_token_embs.weight.requires_grad = False
        unfreeze_special_token_embs = nn.Embedding(self.num_embeddings-freeze_idx_upper, self.embedding_dim)
        unfreeze_special_token_embs.weight.data.copy_(
            ori_embedding.weight.data[freeze_idx_upper:self.num_embeddings]
        )
        unfreeze_special_token_embs.weight.requires_grad = True
        self.weight = torch.cat([freeze_special_token_embs.weight, unfreeze_special_token_embs.weight], dim=0).to(device)
...
class Trainer:
    ...
    def __init__(
        ...
        freeze_flag: Optional[bool] = False,
        **kwargs,
    ):
        if args is None:
        ...
        # Freeze encoder etc. for the half of warmup steps
        self.freeze_flag = freeze_flag
        if self.freeze_flag:
            logger.info(f'Freeze the encoder weights for the first {int(self.args.warmup_steps / 2)} steps!')
            # Freeze the encoder
            for param in model.base_model.encoder.parameters():
                param.requires_grad = False
            for param in model.base_model.embeddings.token_type_embeddings.parameters():
                param.requires_grad = False
            for param in model.base_model.embeddings.position_embeddings.parameters():
                param.requires_grad = False
            logger.info(f'The encoder is frozen!')
            # Freeze part of the special_token_embeddings
            # emb_new = HybroEmbedding(ori_embedding=model.base_model.embeddings.special_token_embeddings, device=self.args.device)
            # model.base_model.embeddings.special_token_embeddings = emb_new
            logger.info(f'Some special tokens are frozen!')
            print('Some special tokens are frozen!')
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    # Reactivate encoder updated
                    if self.freeze_flag and (self.state.global_step >= int(self.args.warmup_steps / 2)):
                        for param in model.parameters():
                            param.requires_grad = True
                        """
                        emb_back = nn.Embedding(6, model.base_model.embeddings.position_embeddings.weight.size(1), padding_idx=1)
                        emb_back.weight.data = model.base_model.embeddings.special_token_embeddings.weight.data
                        emb_back.weight.to(self.args.device)
                        model.base_model.embeddings.special_token_embeddings = emb_back
                        """
                        logger.info(f'All params are unfrozen!')
                        print('All params are unfrozen!')
    ...
    def store_flos(self):
        ...
        if self._total_flos is not None:
            if self.args.local_rank != -1:
                self.state.total_flos = 0#distributed_broadcast_scalars([self._total_flos]).sum().item()
            else:
                self.state.total_flos = 0#self._total_flos
